...
def calendar_month(
    ...
    selected = ymd or now.strftime("%Y-%m-%d")

    cur.execute(
        """
        SELECT id, ymd, author, memo, created_at
        FROM calendar_memo
        WHERE ymd=?
        ORDER BY id DESC
        """,
...
def add_memo(
    ...
    if not memo:
        return RedirectResponse(
            url=f"/page/calendar/month?year={year}&month={month}",
            status_code=303
        )
    ...
    cur.execute(
        """
        INSERT INTO calendar_memo (ymd, author, memo, created_at)
        VALUES (?,?,?,?)
        """,
    ...
    return RedirectResponse(
        url=f"/page/calendar/month?year={year}&month={month}&ymd={ymd}",
        status_code=303
    )

...
async def move_excel(request: Request, file: UploadFile = File(...)):
    ...
    required = ["src_location", "dst_location", "item_code", "item_name", "lot", "spec", "qty"]
    ...
    if missing:
        return templates.TemplateResponse("excel_result.html", {
            ...
            "message": f"필수 컬럼이 없습니다: {', '.join(missing)} (예: 출발로케이션, 도착로케이션, 품번, 품명, LOT, 규격, 수량)",
            "download_url": None,
            "retry_url": "/page/move",
        })
    ...
    with get_db() as conn:
        ...
        for idx, row in enumerate(data_rows, start=2):
            try:
                src_location = get(row, colmap, "src_location")
                dst_location = get(row, colmap, "dst_location")
                ...
                if not all([src_location, dst_location, item_code, item_name, lot, spec]) or qty <= 0:
    ...
    download_url = None
    if fail:
        out_path = os.path.join(tmpdir, f"move_failed_{int(time.time())}.xlsx")

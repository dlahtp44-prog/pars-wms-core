...
@router.get("")
def outbound_form(request: Request, msg: str = ""):
    return templates.TemplateResponse("outbound.html", {"request": request, "title": "출고등록", "msg": msg})
...
def outbound_save(
    ...
    outbound(location, item_code, item_name, lot, spec, int(qty), brand, note)
    return RedirectResponse(url="/page/outbound?msg=저장완료", status_code=303)
...
async def outbound_excel(request: Request, file: UploadFile = File(...)):
    ...
    for r in rows:
        try:
            ...
            outbound(location, item_code, item_name, lot, spec, qty, brand, note)
            ok += 1
    ...
    if bad:
        ...
        token = create_token(err_bytes, filename="outbound_errors.xlsx", content_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
